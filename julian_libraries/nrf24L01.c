...
{
  if      (bEnabled == NRF_ENABLE)   digitalWrite(0, HIGH);
  else if (bEnabled == NRF_DISABLE)  digitalWrite(0, LOW);
}
...
void nrfStartListening(void)
{
  ...
  if ( ! (config & NRF_CONFIG_PWR_UP_bm) ) {
    ...
    usleep(1000 * 2); // delay Power Down --> Standby mode with external oscillator (worst case)
  } else {
    nrfWriteRegister(REG_CONFIG, config|NRF_CONFIG_PRIM_RX_bm);
  }
  usleep(130); // delay Standby --> TX mode
...
void nrfspiInit(void)
{
  ...
  // Set GPIO_PIN as output for NRF_ENABLE
  pinMode(0, OUTPUT);
...
{
  //printf("Transfer %02x,", iData);
  uint8_t data[1] = {iData};
  //printf("%02x\n", data[0]);
